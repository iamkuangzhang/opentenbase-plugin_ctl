...
class CatalogTest(unittest.TestCase):
    ...
    def test_load_smoke_plugin_manifest_from_examples(self) -> None:
        root = Path(__file__).resolve().parents[1]
        catalog = Catalog(root=root)
        manifest = catalog.load_one("pluginctl_smoke_plugin")
        self.assertEqual(manifest.plugin_id, "pluginctl_smoke_plugin")
        self.assertEqual(manifest.source_root, root / "examples" / "plugins" / "pluginctl_smoke_plugin" / "payload")
