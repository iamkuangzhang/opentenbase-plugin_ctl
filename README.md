...
Create a beginner-friendly OpenTenBase / PostgreSQL extension skeleton:
...
`dev init` generates a small SQL-only plugin directory with Chinese comments, `manifest.yml`, a `.control` file, install SQL, verify SQL, rollback SQL, `.pluginctlignore`, and a starter README. It is meant for learning the basic extension layout and PluginCtl lifecycle flow; it does not compile C code or generate `.so` files.

You can choose a parent directory or overwrite files generated by this command:
