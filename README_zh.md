...
回滚默认是 dry-run。只有明确要执行回滚 SQL 时才加 `--execute`：
...
PluginCtl Shell 是插件生命周期控制台，用来管理插件的发现、检查、部署、验证、回滚和报告。它不是 OpenTenBase 集群控制台，不负责集群启动、停止、初始化或监控。
...
- `deploy -f cluster.toml` 默认 dry-run，只展示物理分发计划。
...
- `deploy <plugin_id>` 会执行本地安装 SQL。
- `deploy <plugin_id> --execute` 会通过 `scp` 分发远程文件。
- `register <plugin_id> --execute` 会在 primary coordinator 上执行 `CREATE EXTENSION`。
- `rollback <plugin_id> --execute` 会执行 manifest 声明的回滚 SQL。
